...
namespace firopow_main
{
constexpr auto revision = FIROPOW_REVISION;

static constexpr int epoch_length = FIROPOW_EPOCH_LENGTH;
static constexpr int light_cache_item_size = FIROPOW_LIGHT_CACHE_ITEM_SIZE;
static constexpr int full_dataset_item_size = FIROPOW_FULL_DATASET_ITEM_SIZE;
static constexpr int num_dataset_accesses = FIROPOW_NUM_DATASET_ACCESSES;

using epoch_context = firopow_epoch_context;
using epoch_context_full = firopow_epoch_context_full;

using result = firopow_result;
...
/// Alias for firopow_calculate_light_cache_num_items().
static constexpr auto calculate_light_cache_num_items = firopow_calculate_light_cache_num_items;

/// Alias for firopow_calculate_full_dataset_num_items().
static constexpr auto calculate_full_dataset_num_items = firopow_calculate_full_dataset_num_items;

/// Alias for firopow_calculate_epoch_seed().
static constexpr auto calculate_epoch_seed = firopow_calculate_epoch_seed;
...
using epoch_context_ptr = std::unique_ptr<epoch_context, decltype(&firopow_destroy_epoch_context)>;

using epoch_context_full_ptr =
    std::unique_ptr<epoch_context_full, decltype(&firopow_destroy_epoch_context_full)>;

/// Creates Ethash epoch context.
///
/// This is a wrapper for firopow_create_epoch_number C function that returns
...
{
    return {firopow_create_epoch_context(epoch_number), firopow_destroy_epoch_context};
}

inline epoch_context_full_ptr create_epoch_context_full(int epoch_number) noexcept
{
    return {firopow_create_epoch_context_full(epoch_number), firopow_destroy_epoch_context_full};
}
...
{
    return firopow_hash(&context, &header_hash, nonce);
}
...
{
    return firopow_verify_final_hash(&header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return firopow_verify(&context, &header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return *firopow_get_global_epoch_context(epoch_number);
}

/// Get global shared epoch context with full dataset initialized.
inline const epoch_context_full& get_global_epoch_context_full(int epoch_number) noexcept
{
    return *firopow_get_global_epoch_context_full(epoch_number);
}
}  // namespace firopow_main
