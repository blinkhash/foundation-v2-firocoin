...
// Verify constants:
static_assert(sizeof(ethash::hash512) == FIROPOW_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(sizeof(ethash::hash1024) == FIROPOW_FULL_DATASET_ITEM_SIZE, "");
static_assert(light_cache_item_size == FIROPOW_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(full_dataset_item_size == FIROPOW_FULL_DATASET_ITEM_SIZE, "");
    ...
    build_light_cache_fn build_fn, int epoch_number, bool full) noexcept
{
    ...
    for (uint32_t i = 0; i < firopow_progpow::l1_cache_size / sizeof(full_dataset_2048[0]); ++i)
...
using namespace firopow_main;

extern "C" {

ethash_hash256 firopow_calculate_epoch_seed(int epoch_number) noexcept
{
...
epoch_context* firopow_create_epoch_context(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, false);
}

epoch_context_full* firopow_create_epoch_context_full(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, true);
}

void firopow_destroy_epoch_context_full(epoch_context_full* context) noexcept
{
    firopow_destroy_epoch_context(context);
}

void firopow_destroy_epoch_context(epoch_context* context) noexcept
{
    context->~epoch_context();
    std::free(context);
}

firopow_result firopow_hash(
...
bool firopow_verify_final_hash(const ethash::hash256* header_hash, const ethash::hash256* mix_hash, uint64_t nonce,
...
bool firopow_verify(const epoch_context* context, const ethash::hash256* header_hash,
